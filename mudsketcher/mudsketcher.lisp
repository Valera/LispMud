...
;		 (store-add-item m "Собака")
		 ...
		 (destructuring-bind
		       (&key name enter-verb leave-verb move-interval animation-1 animation-2
			     animation-1-timer animation-2-timer)
...
;		 (declare (ignore c))
		 (setf (widget-sensitive mob-editor-optons-button)
	       ...
	       (initialize-banker-dialog (dialog)
		 (dialog-add-button dialog "gtk-cancel" :cancel)
		 (dialog-add-button dialog "gtk-new" :accept))
	       (run-banker-dialog (builder dialog-name fields-plist)
		 (let ((dialog (builder-get-object builder dialog-name)))
		   ...
		   (let ((dialog-result (dialog-run dialog)))
			 ...
			 (iter (with result)
			       (for (field-key widget-name) on fields-plist by #'cddr)
			       (appending (list field-key (text-buffer-text (text-view-buffer (builder-get-object builder widget-name)))))
			       (finally (return-from run-banker-dialog result)))
			 nil))))
	       (room-type-changed (c)
		 (declare (ignore c))
		 ...
		 (setf (lispmud::short-description room) (entry-text entry)
	       ...
	       (initialize-options-dialogs ()
		 (iter (for d in (list (builder-get-object builder "bank-options-dialog")))
		       (dialog-add-button d "gtk-cancel" :cancel)
		       (dialog-add-button d "gtk-save" :accept)))
...
;			     (list 
	...
	(initialize-banker-dialog (builder-get-object builder "banker-mob-options-dialog"))
	...
	(connect-signal mob-editor-optons-button "clicked" #'(lambda (b) (declare (ignore b))
								     (run-banker-dialog builder "banker-mob-options-dialog"
